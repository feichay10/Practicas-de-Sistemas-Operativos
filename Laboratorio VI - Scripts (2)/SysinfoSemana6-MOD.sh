...
# Hacer una función que dijera cuántos procesos hay en ejecución 
# de cada usuario que hay en el fichero /etc/passwd
procesospasswd()    # $USERS_SYSTEM=$(cat /etc/passwd | tr ':' ' ' | awk '{ print $1 }')
{
  echo "${TEXT_ULINE}${TEXT_BOLD}${TEXT_YELLOW}MOD${TEXT_RESET}"
  for i in $USERS_SYSTEM; do
    printf "%s" "$i " "-> " "$(ps -u $i --no-headers | wc -l)"
    echo
  done
}

# Hacer una funcion que dijera los usuarios de /etc/passwd 
# que tengan mas de un proceso
procesospasswd2()    # $USERS_SYSTEM=$(cat /etc/passwd | tr ':' ' ' | awk '{ print $1 }')
{
  echo "${TEXT_ULINE}${TEXT_BOLD}${TEXT_YELLOW}MOD_2${TEXT_RESET}"
  for i in $USERS_SYSTEM; do
    if [ "$(ps -u $i --no-headers | wc -l)" -gt 0 ]; then
      printf "%s" "$i " "-> " "$(ps -u $i --no-headers | wc -l)"
      echo
    fi
  done
