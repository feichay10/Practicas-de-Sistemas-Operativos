...
# Entregable: Semana 6 Sysinfo Modificacion

# sysinfo - Un script que informa del estado del sistema

# Modificacion: mirar cuales de los usuarios de etc/passwd no estan ejecutando ningun proceso
...
# Modificacion: mirar cuales de los usuarios de /etc/passwd no estan ejecutando ningun proceso
# Ejecutar ./SysinfoSemana6-Modificacion.sh -o para ver esta funcion
procesospasswd()
{
  USERS_PASSWD=$(cat /etc/passwd | tr ':' ' ' | awk '{print $1}')
  for i in $USERS_PASSWD; do
    if [ $(ps -u $i --no-headers | wc -l) == "0" ]; then
      echo $i
    fi
  done
}
